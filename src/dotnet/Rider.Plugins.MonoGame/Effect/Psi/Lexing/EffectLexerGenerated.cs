...
public partial class EffectLexerGenerated
{
    private readonly Func<IBuffer, ILexer> _myCppLexerFactory = null;
    private readonly ReusableBufferRange _myBufferRange = new ReusableBufferRange();
    ...
    public EffectLexerGenerated(IBuffer buffer, Func<IBuffer, ILexer> cppLexerFactory) : this(buffer)
    {
        _myCppLexerFactory = cppLexerFactory;
    }
    ...
    private TokenNodeType LocateToken()
    {
        if (currentTokenType == null)
        {
            try
            {
                ...
                if (currentTokenType == EffectTokenType.EFFECT_CODE && _myCppLexerFactory != null)
                {
                    myActiveCppLexer =
                        _myCppLexerFactory(ProjectedBuffer.Create(yy_buffer, new TextRange(TokenStart, TokenEnd)));
    ...
    // {
    //     return Keywords.GetValueSafe(_myBufferRange, yy_buffer, yy_buffer_start, yy_buffer_end);
    // }
    //
